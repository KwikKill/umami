...
describe('getChannelExpandedMetrics postgres branch', () => {
  test('builds the expanded aggregation query with parseFilters fragments', async () => {
    ...
    expect(query).toContain('sum(case when visit_stats.c = 1 and coalesce(visit_events.has_custom_event, 0) = 0 and coalesce(visit_events.has_heartbeat, 0) = 0 then 1 else 0 end) as "bounces"');
...
describe('getChannelExpandedMetrics clickhouse branch', () => {
  ...
  test('builds the multiSearchAny aggregation query', async () => {
    ...
    expect(query).toContain('sumIf(1, t.c = 1 and ifNull(e.has_custom_event, 0) = 0 and ifNull(e.has_heartbeat, 0) = 0) as "bounces"');
